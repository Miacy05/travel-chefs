...
地图 / 图鉴几张为了让画面有内容，脚本先通过公开接口造了一份"玩过一阵子"的存档（`tools/shot.js --seed=1`），页面本身仍是真实渲染。
...
| 图鉴 · 菜谱 | 图鉴 · 徽章 | 店铺升级 |
| --- | --- | --- |
| <img src="docs/codex.png" width="232" alt="图鉴菜谱"> | <img src="docs/badges.png" width="232" alt="图鉴徽章"> | <img src="docs/upgrade.png" width="232" alt="升级"> |
| 菜谱 / 顾客 / 徽章 / 明信片 四个分类切换 | 每地区 2 枚，全通点亮；困难徽章待困难模式开放 | 4 条升级线，全部用游戏内金币购买 |
...
4. **出餐**：
   - 按配方把食材**逐个点进锅**（备料点齐即可，不要求顺序）；食材旁的小字会标出「这配料能用来做本关哪几道菜」；
   - 想更快，可以点**「一键下锅」**把当前订单的配料一次下完——「做菜 → 装盘 → 上菜」压成三次点击；
   - 点灶台上的锅**开火**，等待烹饪进度条走完；**多灶台默认只点亮一口**，每上满 5 道菜多点亮一口，没点亮的灶画成暗色小锁；
   - 把锅里的菜**拖到菜盘**完成装盘；
   - 把盘子**拖给对应顾客**完成上菜。
...
平时收在一个「🧰 道具」开关后面（带待用数量角标），点开才展开二级菜单——常驻按钮从 3 个降到 1 个：
...
每个地区普通 5 关**全部通关**后，关卡列表底部会出现「🔥 挑战模式」入口。困难关卡复用普通关卡的关卡号，难度来自四重叠加：
...
| 上菜更密 | 上客间隔缩短到 70% |
| 菜更多 | 可选菜品 +1 道，更难同时备对 |
| 灶台会坏 | 正在煮的锅随机「跳闸」，进度回退 25%，有冷却保护 |

困难关卡结算写**独立的困难星级**（不影响普通进度），该地区困难关卡全通后获得**困难徽章**。
...
| 关 | 名称 | 时长 | 座位 | 上客间隔 | 可用菜数 | 三星条件（服务 / 收入 / 流失 / 连击） |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 夜市开张 | 90s | 2 | 6.8s | 1 道 | 7 位 / 150 / ≤1 / ≥6 |
| 2 | 新菜上架 | 90s | 2 | 6.0s | 2 道 | 8 位 / 210 / ≤1 / ≥7 |
| 3 | 人气爆棚 | 100s | 2 | 5.4s | 3 道 | 10 位 / 290 / ≤1 / ≥8 |
| 4 | 晚高峰 | 110s | 3 | 4.8s | 3 道 | 12 位 / 380 / ≤2 / ≥10 |
| 5 | 美食节 | 120s | 3 | 4.2s | 3 道 | 15 位 / 500 / ≤2 / ≥12 |

> 表内收入目标是第 1 区（×1.00）的基准值。第 5 区 ×1.60，所以 E5 的收入目标是 800。

**关于「等待时间 ×2」**：所有顾客的耐心统一乘以 `CONFIG.patienceMul = 2`（最短的快递员也有
32 秒，最耐心的老奶奶有 80 秒），新手不会被时间逼死。为了不让三星变成白送，挑战性挪到了三处：
上客节奏变快（顾客来得更密，座位与出餐吞吐成为真瓶颈）、服务量与收入门槛各提高约 15%~25%、
最高连击门槛提高（连击只看 Perfect，与耐心无关，是真正吃操作的那条）。这三条同时受
`tests/difficulty.test.js` 的双向守卫——既不许调到「三星打不出来」，也不许松到「随便过」。
...
### 8 种顾客（× 5 个地区 = 40 种本地化造型）

每位顾客的立绘是**程序化合成**的：共用一张 16×24 线稿，叠上「职业补丁」（帽子、围裙、
双马尾、相机……），再按所在地区上色（巴黎是金发 + 薄荷绿，墨西哥是红衣 + 奶油饰）。
职业本身没戴帽子的，会改戴当地帽子——同一职业在不同城市一眼能认出来。

| 顾客 | 耐心 | 小费倍率 | 职业特征 | 特点 |
| --- | --- | --- | --- | --- |
| 快递员 | 16s | ×1.15 | 深色头盔 + 反光条 | 最急，但给得多 |
| 学生党 | 18s | ×0.9 | 鸭舌帽 + 短袖 | 手头紧 |
| 小朋友 | 20s | ×1.2 | 双马尾 + 背带裤 | 急但给得多 |
| 上班族 | 24s | ×1.0 | 短袖衬衫 + 领带夹 | 标准 |
| 同行厨师 | 26s | ×1.25 | 高帽 + 白围裙 | 挑手艺 |
| 家庭主妇 | 28s | ×1.05 | 发带 + 围裙 | 不紧不慢 |
| 背包游客 | 30s | ×1.35 | 胸前挂相机 | 爱拍照、给得多 |
| 老奶奶 | 40s | ×1.1 | 银发 + 老花镜 + 披肩 | 最耐心 |

同屏最多 **6 位**（座位上限也是 6），排成 **2 行 3 列**：1~3 人就占一行，第 4 位开始
自动折到第二行并把立绘缩到 2 倍像素，新顾客从右侧滑入、上完菜向左滑走，不会挤成一团。

### 4 种特殊顾客（带金色发光描边）

特殊顾客作为**额外顾客**加入本局（不替换普通顾客），入场即激活 30 秒专属道具：

| 特殊顾客 | 触发条件 | 入场道具 | 特点 |
| --- | --- | --- | --- |
| 🧙 神秘老人 | 累计服务 50 位 | 耐心不再减少 | 传授祖传秘方 |
| 👨‍🍳 美食评论家 | 累计服务 150 位 | 金币 +10% | 满意给巨额小费 |
| 📸 旅行博主 | 每个自然日首次开局 | 烹饪加速 50% | 边吃边拍 |
| 🐈 流浪猫 | 随机（困难徽章越多概率越高） | 小费 ×1.5 | 蹭饭带来好运 |

每种特殊顾客**一生只出现一次**（旅行博主例外，每天一次）。它们在顾客带里带金色发光描边、
专属图标，图鉴「顾客」页里也以金框展示。
...
> **装修与座位都是「分地区」的**：加座 / 招牌灯箱 / 装饰布置的等级按地区分别记，
> 在哪个地区买就只对哪个地区生效。每个地区还有自己的 Lv1–Lv3 专属装饰件，
> Lv4–Lv5 追加发光描边；买完当帧生效，不用重开一局。

服务员只有一条线，三级各不同：

| 等级 | 效果 |
| --- | --- |
| Lv1 | 每 **5 秒**自动上菜一次，优先照顾**耐心最低**的顾客 |
| Lv2 | 每 **3 秒**自动上菜 · **手动上菜小费 +10%** |
| Lv3 | 每 **2 秒**自动上菜 · 每服务 **5 位**顾客，**全体顾客耐心 +10%** |

> 早期版本还有「帮厨」「收银员」两条员工线，本轮已简化为只保留服务员 ——
> 旧存档里买过这两项的，进游戏时会一次性把金币退回到账（`staffRefunded` 闸门保证只退一次）。
...
- **图鉴**：页内分四个分类切换（**菜谱 / 顾客 / 徽章 / 明信片**），一次只显示一类，不需要在一条长列表里翻。
  - *菜谱*：已解锁显示菜名 + 配料，未解锁显示「？？？」。
  - *顾客*：未遇见的画成灰色剪影（同一轮廓、去掉配色），特殊顾客带金色边框；遇见过多次的会显示「已遇见 X 次」。
  - *徽章*：每个地区 2 枚（普通通关 / 困难通关），共 10 枚。未获得的显示灰色 + 「完成 XX 全部 N 关解锁」；困难徽章带实际效果（永久解锁流浪猫光顾）。
  - *明信片*：每个地区一张像素明信片，纯收藏。
...
| 彩蛋 | 触发方式 | 奖励 |
| --- | --- | --- |
| 🌏 地球仪 | 在地图上连点地球仪 10 次 | 50 金币（**每个自然日一次**，跨天计数归零；当天再点会提示「明天再来」） |
| 🌈 彩虹糖 | 单局连续 5 次 Perfect 上菜 | 彩虹特效 |
| 🐧 企鹅厨师 | 在厨房里连点冰箱 5 次 | 1 钻石（每局限一次） |

> 地图页那颗地球仪是**程序化画出来的**（不是静态图片）：球面按经纬度着色，
> 大陆 / 海岸 / 云层都跟着球体慢转；外面还有一圈点状轨道、几颗星星和一架绕圈飞的小飞机。
> 同一个球在任意角度都能算出正确画面，所以只需一张 32×32 的逻辑网格 + 4px 方块。
> 想单独看一眼效果：`node tools/globe-preview.js`（字符画）或 `node tools/globe-png.js`（导出 PNG）。

---

## 四·补、体验增强（第九优先级新增）

在核心玩法之上，这一批改动专注于「让游戏更好上手、反馈更清晰」：
...
| 👉 下一步提示 | 画面自动用跳动小箭头 + 金色描边指出「现在最该点的位置」 |
| 😊 满意度 | 结算页新增「顾客满意度」= 成功上菜 ÷ 总上门顾客 |
| 🎁 每日登录奖励 | 每天首次进入发金币，连签递增（20 → 25 → … → 50 封顶），断签重来 |
| ↻ 一键重玩 | 地图页一键回到上一关，免去反复翻地区 |
| 📊 地区进度条 | 地图页显示「还差几颗星解锁下一个地区」 |
| 🔉 音量调节 | 设置页新增音量滑块，所有音效实时调响度 |
| 🏆 成就全屏动画 | 达成成就时弹出带光芒的全屏横幅，可点击跳过 |

> 以上每一项都有对应单元测试（`tests/features.test.js` 及其它分项文件），规则层 100% 可回归。
...
- 地图上的地球仪改用**球面参数化程序绘制**（`TC.Pixel.globeBit`）：给一个网格坐标和自转角就返回色号，
  于是"自转"只是角度在变，而大陆、经纬线、云层会一起正确地绕到背面去；
  球外的虚线轨道 / 星星 / 小飞机也用同一套网格坐标算，对齐问题一并解决。
- 全部动画只在**当前真的看得见**时才跑：离开地图页、页面切到后台都会停掉 `requestAnimationFrame`；
  系统开了「减少动态效果」只画一帧静态球。
...
`TC.Audio` 用 **WebAudio 实时合成**（15 种音色全是振荡器 + 包络），不引入任何音频文件。
...
| `badge.test.js` | 徽章数据表与「全通才发」的判定 |
| `calc.test.js` | 价格、小费、星级、经验、结算 |
| `codex.test.js` | 图鉴四个分类的渲染与切换、顶栏文案 |
| `customer.test.js` | 顾客职业表、按地区换装、2 行 3 列排队 |
| `daily.test.js` | 每日任务抽取与进度 |
| `data.test.js` | 数据表自洽性（关卡/菜品/升级索引） |
| `difficulty.test.js` | 难度曲线：等待时间 ×2、三星「打得出也咬得人」 |
| `easter.test.js` | 三个彩蛋；地球仪「每天一次」的同日锁定 / 跨天重置 |
| `features.test.js` | 第九优先级：登录奖励、满意度、一键重玩、音量、下一步提示 |
| `flow.test.js` | 端到端：开一关 → 做单 → 上菜 → 结算 → 解锁 |
| `globe.test.js` | 地球仪球面算法（色号合法、描边完整、旋转周期、飞机不越界） |
| `guest.test.js` | 特殊顾客：触发判定、一生一次、每日首登、30 秒道具、结算加成 |
| `hard.test.js` | 挑战模式：解锁判定、独立星级、灶台故障、困难徽章 |
| `level.test.js` | 关卡状态机（备料/烹饪/装盘/上菜/道具） |
| `pixel.test.js` | 像素图形生成与 5 色校验（含 40 种顾客造型不撞款） |
| `router.test.js` | 视图路由与顶栏 |
| `save.test.js` | 存档读写、归一化、迁移 |
| `scene.test.js` | 布局、命中检测、绘制不变量 |
| `simplify.test.js` | 一键下锅、多灶台逐步点亮、道具栏折叠、食材可做菜小字 |
| `tutorial.test.js` | 新手教程：步骤表、教学局规则、可重看 |
