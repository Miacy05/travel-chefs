...
- **图鉴**：页内分四个分类切换（**菜谱 / 顾客 / 徽章 / 明信片**），一次只显示一类，不需要在一条长列表里翻。
  - *菜谱*：已解锁显示菜名 + 配料，未解锁显示「？？？」。
  - *顾客*：未遇见的画成灰色剪影（同一轮廓、去掉配色），特殊顾客将来会带金色边框。
  - *徽章*：每个地区 2 枚（普通通关 / 困难通关），共 10 枚。未获得的显示灰色 + 「完成 XX 全部 N 关解锁」。
  - *明信片*：每个地区一张像素明信片，纯收藏。
